...
import com.example.project.db.repository.NotificationRepository;
...
import static com.example.project.api.controller.SseController.sseEmitters;
...
public class NotificationServiceImpl implements NotificationService {
    ...
    @Override
    public List<NotificationResp> selectAllNotification(Long userId) {


        List<Notification> notifications = notificationRepository.findTop30ByUserIdOrderByCreatedAtDesc(userId); // 현재 로그인한 user의 알림

        List<NotificationResp> notificationList = new ArrayList<>(); // 알림 목록을 담을 list
        ...
        for (Notification noti : notifications) {
            ...
            if (type.equals("REVIEW")) { // 댓글일 때
                notificationList.add(NotificationResp.builder()
